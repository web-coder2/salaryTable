...
createApp({
  ...
  computed: {
    monthlyCalculations() {
      ...
      this.calculations.forEach(calc => {
        ...
        monthlyData[month].total += calc.summHold; // Sum "SummHold" for each month
      });
    ...
    yearlyCalculations() {
      ...
      this.calculations.forEach(calc => {
        ...
        yearlyData[year].total += calc.summHold; // Sum "SummHold" for each year
      });
  ...
  methods: {
    async addRecord() {
      this.isAdding = true; // Start animation
      try {
        ...
        setTimeout(() => {
          this.isAdding = false; // End animation after 300ms
        }, 300);
      }
    },
    resetForm() {
    ...
    getOfficeSalaryColor(officeSalary) {
      return officeSalary < 0 ? 'rgba(255, 0, 0, 0.1)' : 'rgba(0, 255, 0, 0.1)';
    },
